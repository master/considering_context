...
class TwoHot:
    ...
    def mode(self):
        ...
        if n % 2 == 1:
            ...
            b2 = self.bins[..., m:m + 1]
            b3 = self.bins[..., m + 1:]
            wavg = (p2 * b2).sum(dim=-1, keepdim=True) + ((p1 * b1).flip(dims=(-1,)) + p3 * b3).sum(dim=-1, keepdim=True)
    ...
    def log_prob(self, target):
        ...
        equal = below == above
        dist_to_below = torch.where(equal, torch.tensor(1.0, device=target.device, dtype=torch.float32), (self.bins[below] - target_squashed).abs())
        dist_to_above = torch.where(equal, torch.tensor(1.0, device=target.device, dtype=torch.float32), (self.bins[above] - target_squashed).abs())
...
class SymlogDist:
    ...
    def __init__(self, mode, dist='mse', agg='sum', tol=1e-08):
