...
from torch.utils._foreach_utils import _device_has_foreach_support, _group_tensors_by_device_and_dtype, _has_foreach_support
...
def clip_grad_agc_(parameters: _tensor_or_tensors, clip: float, pmin: float, foreach: bool | None=None):
    ...
    if len(grads) == 0:
        return
    grouped: dict[tuple[torch.device, torch.dtype], tuple[list[list[Tensor]], list[int]]] = _group_tensors_by_device_and_dtype([params, grads])
